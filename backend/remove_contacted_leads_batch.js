...
async function removeContactedLeads() {
  ...
  // Step 2: Find all lead IDs that have been contacted
  ...
  // Step 3: Get details of contacted leads in batches
  console.log('📋 Fetching contacted leads from this list...');

  const contactedLeadsInList = [];
  ...
  for (let i = 0; i < contactedLeadIds.length; i += batchSize) {
    ...
    if (batchError) {
      console.error(`❌ Failed to fetch batch ${Math.floor(i / batchSize) + 1}:`, batchError);
    } else if (batchLeads && batchLeads.length > 0) {
      contactedLeadsInList.push(...batchLeads);
      console.log(`   Batch ${Math.floor(i / batchSize) + 1}: Found ${batchLeads.length} contacted leads`);
    }
  }
  ...
  let totalDeleted = 0;
  const deleteBatchSize = 50; // Delete 50 at a time

  for (let i = 0; i < leadIdsToDelete.length; i += deleteBatchSize) {
    const batch = leadIdsToDelete.slice(i, i + deleteBatchSize);
    ...
    if (deleteError) {
      console.error(`❌ Failed to delete batch ${Math.floor(i / deleteBatchSize) + 1}:`, deleteError);
    } else {
      totalDeleted += count || 0;
      console.log(`   Batch ${Math.floor(i / deleteBatchSize) + 1}: Deleted ${count || 0} leads`);
    }
  }
